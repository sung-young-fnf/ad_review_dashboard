...
class Menu(Base):
    ...
    parent_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("{{APP_NAME_SNAKE}}.menus.id"))
...
class MenuPermission(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    menu_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("{{APP_NAME_SNAKE}}.menus.id", ondelete="CASCADE"))
    role_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("{{APP_NAME_SNAKE}}.roles.id", ondelete="CASCADE"))
