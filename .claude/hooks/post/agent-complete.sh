...
# Claude Code 2.1.2: session-start-loader.sh와 일관된 기본값 사용
event_info=$(cat)
SESSION_ID=$(echo "$event_info" | jq -r '.session_id // "unknown"' 2>/dev/null || echo "unknown")
...
# 2.1.47 신규 필드: last_assistant_message (에러 감지 정확도 향상)
...
# Step 1.5: 에러 감지 (2.1.47: last_assistant_message 우선, transcript fallback)
ERROR_COUNT=0
ERROR_SUGGESTION=""

# 1순위: last_assistant_message에서 에러 감지 (가장 정확, 오탐 최소화)
if [[ -n "$LAST_ASSISTANT_MSG" ]]; then
  ERROR_COUNT=$(echo "$LAST_ASSISTANT_MSG" | grep -ciE "(ERROR:|TypeError:|SyntaxError:|Build failed|FAILED|컴파일 에러)" 2>/dev/null || echo 0)
  if [[ "$ERROR_COUNT" -gt 0 ]]; then
    ERROR_SUGGESTION="⚠️ 에이전트 최종 응답에서 에러 ${ERROR_COUNT}건 감지\n\n💡 error-fixer로 자동 수정 권장:\nTask(subagent_type: 'error-fixer', run_in_background: true, model: 'haiku', prompt: 'Fix errors from $AGENT_TYPE')"
  fi

# 2순위: transcript 파일 파싱 (fallback, 2.0.42~2.1.46)
elif [[ -n "$TRANSCRIPT_PATH" ]] && [[ -f "$TRANSCRIPT_PATH" ]]; then
  ERROR_COUNT=$(grep -ciE "(ERROR|FAIL|Exception|Error:|TypeError|SyntaxError)" "$TRANSCRIPT_PATH" 2>/dev/null || echo 0)
  if [[ "$ERROR_COUNT" -gt 0 ]]; then
    ERROR_SUGGESTION="⚠️ 에이전트 실행 중 에러 ${ERROR_COUNT}건 감지\n\n💡 error-fixer로 자동 수정 권장:\nTask(subagent_type: 'error-fixer', run_in_background: true, model: 'haiku', prompt: 'Fix errors from $AGENT_TYPE')"
  fi
fi
...
if [ -z "$MODIFIED_FILES" ]; then
  echo "⚠️ 변경된 파일 없음. Impact 기록 스킵." >&2
  cat << 'EOF'
{"continue": true}
EOF
  exit 0
...
EOF
    ...
    # 파일로 handoff 정보 저장 (claude mcp는 bash에서 사용 불가)
    echo "$HANDOFF_CONTENT" > "$STATE_DIR/handoff_${NEXT_TASK_ID}.json" 2>/dev/null || true

    echo "💾 Handoff 저장: $STATE_DIR/handoff_${NEXT_TASK_ID}.json" >&2
  else
    echo "" >&2
...
# ============================================================================
# praetorian/praetorian_compact 리마인더 (CLAUDE.md 필수 트리거)
# ============================================================================
echo "" >&2
echo "📦 MANDATORY: praetorian/praetorian_compact 호출 필수!" >&2
echo "   → type: \"task_result\", title: \"${AGENT_TASK:-Agent완료}\"" >&2
echo "" >&2

# ============================================================================
# implementation-validator 리마인더 (code-writer 완료 시)
# ============================================================================
VALIDATOR_MSG=""
if [[ "$AGENT_TYPE" == *"code-writer"* ]]; then
  VALIDATOR_MSG="⚠️ MANDATORY: implementation-validator 호출 필수!\\n→ Task(subagent_type: 'implementation-validator', prompt: 'Task ${AGENT_TASK:-완료된Task} AC 검증')"
  echo "" >&2
  echo "🔍 MANDATORY: implementation-validator 호출 필수!" >&2
  echo "   → Task(subagent_type: 'implementation-validator', prompt: 'Task ${AGENT_TASK:-완료된Task} AC 검증')" >&2
  echo "" >&2
fi
...
# 2.0.42: transcript 분석 결과 포함
# ============================================================================

# systemMessage 조합 (에러 + validator 리마인더)
COMBINED_MSG=""
if [[ -n "$ERROR_SUGGESTION" ]]; then
  COMBINED_MSG="$ERROR_SUGGESTION"
fi
if [[ -n "$VALIDATOR_MSG" ]]; then
  if [[ -n "$COMBINED_MSG" ]]; then
    COMBINED_MSG="${COMBINED_MSG}\\n\\n${VALIDATOR_MSG}"
  else
    COMBINED_MSG="$VALIDATOR_MSG"
  fi
fi
